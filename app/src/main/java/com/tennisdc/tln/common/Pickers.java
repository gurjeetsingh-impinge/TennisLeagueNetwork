...
public class Pickers {
	...
	public static class MatchDatePickerFragment extends DialogFragment implements DatePickerDialog.OnDateSetListener {


		public static MatchDatePickerFragment newInstance(long time) {
			Bundle bundle = new Bundle();
			bundle.putLong("DATE", time);

			MatchDatePickerFragment dialog = new MatchDatePickerFragment();
		...
		public Dialog onCreateDialog(Bundle savedInstanceState) {
			...
			if (null != getArguments()) {
				timeDefault = getArguments().getLong("DATE");
			}
			if (timeDefault > 0) c.setTimeInMillis(timeDefault);
		...
		@Override
		public void onDateSet(DatePicker datePickerView, int i, int i1, int i2) {
			if (getTargetFragment() instanceof FragSubmitScorePager.FragSubmitCompetitionDetails) {
				((FragSubmitScorePager.FragSubmitCompetitionDetails) getTargetFragment()).onDatePicked(datePickerView, i, i1, i2);
			}
		}
